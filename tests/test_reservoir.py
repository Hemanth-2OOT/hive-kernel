...
def main():
    ...
    print("Starting sentiment cell (measuring OS Popen latency)...")
    t0 = time.time()
    res.start_sentiment_cell()
    t1 = time.time()
    print(f"Cell OS Popen completed in {t1-t0:.2f}s")
    
    print("\nSending Request 1 (Cold Start: Includes PyTorch Import + Model Load)...")
    t2 = time.time()
    resp1 = res.infer_sentiment(1, "I love this architecture!")
    t3 = time.time()
    ...
    t4 = time.time()
    resp2 = res.infer_sentiment(2, "This is garbage.")
    t5 = time.time()
    ...
    t6 = time.time()
    resp3 = res.infer_sentiment(3, "Whatever.")
    t7 = time.time()
